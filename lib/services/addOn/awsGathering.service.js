...
let currentConfig;
const AwsImports = __importStar(require("./imports/awsPackage.import"));
////////////////////////////////////////////////////////////////////////////////////////////////////////
...
async function collectData(awsConfig) {
    ...
    for (let oneConfig of awsConfig ?? []) {
        ...
        try {
            ...
            let awsSessionToken = await (0, manageVarEnvironnement_service_1.getConfigOrEnvVar)(oneConfig, "AWS_SESSION_TOKEN", prefix);
            if (awsSessionToken)
                (0, manageVarEnvironnement_service_1.setEnvVar)("AWS_SESSION_TOKEN", awsSessionToken);
            if (awsKeyId)
                (0, manageVarEnvironnement_service_1.setEnvVar)("AWS_ACCESS_KEY_ID", awsKeyId);
            else
                logger.warning(prefix + "AWS_ACCESS_KEY_ID not found");
            if (awsSecretKey)
                (0, manageVarEnvironnement_service_1.setEnvVar)("AWS_SECRET_ACCESS_KEY", awsSecretKey);
            else
                logger.warning(prefix + "AWS_SECRET_ACCESS_KEY not found");
            const credentialProvider = (0, credential_providers_1.fromNodeProviderChain)();
            const client = new client_ec2_2.EC2Client({ region: "us-east-1", credentials: credentialProvider });
            ...
            if (response.Regions) {
                ...
                const promises = response.Regions.map(async (region) => {
                    try {
                        ...
                        let newResources = await collectAuto(credentialProvider, region.RegionName);
                        const newCustomResources = await collectCustom(credentialProvider, region.RegionName);
                        newCustomResources.forEach((customRes) => {
...
;
const updatePkgImport_service_1 = require("../updatePkgImport.service");
function extractObjectsOrFunctions(module, isObject) {
    const clients = {};
    let clientsMatch = [];
    let clientsResults = [];
        ...
        if ((module[key] instanceof Function && module[key].prototype !== undefined && module[key].name.endsWith("Client"))) {
            ...
            if (clientName != "Client") {
                ...
                if (clientsMatch.length > 1)
                    logger.warning("WARNING: Multiple client found for AWS objects, gather could be wrong.");
                else if (clientsMatch.length < 1)
                    logger.warning("WARNING: No client found for AWS objects, gather could be wrong.");
            }
        }
    });
    Object.keys(module).forEach((key) => {
        if ((module[key] instanceof Function && module[key].prototype !== undefined
            ...
            if (isObject) {
                const objectName = (0, updatePkgImport_service_1.extractObjectBetween)(module[key].name, startStrings, endString);
                if (clientsMatch.length < 1)
                    ...
                    clientsResults.push({ clientName: clientsMatch[clientsMatch.length - 1].name,
                        ...
                        objectName: objectName, objectFunc: module[key] /*,  input  ,  output */ });
...
function concatAllObjects(collectedResults) {
    let finalResults = {};
    collectedResults.forEach((element) => {
...
let iamUsers;
async function collectAuto(credential, region) {
        ...
        for (const dependence of awsGatherDependencies) {
            if (Array.isArray(dependence.functions)) {
                for (let i = 0; i < dependence.functions.length; i++) {
                    ...
                    dependence.objects.forEach((element) => {
    ...
    for (const client of objectToGather) {
        const promises = client.map(async (object) => {
...
function gatherDependenciesResources(credential, region, object) {
}
async function gatherAwsObject(credential, region, object) {
                    ...
                    if (dependence.matchingError == error.constructor.name) {
                        ...
                        for (const obj of dependence.objects) {
                            ...
                            if (Array.isArray(obj.results)) {
                                const objPromises = obj.results.map(async (objToTest) => {
                                    try {
...
async function collectCustom(credential, region) {
    let azureRet;
    azureRet = await Promise.all(ressource_models_1.stringKeys.map(async (element) => {
...
const customGatherFunctions = {
    'KexaAwsCustoms.tagsValueListing': async (credential, region, object) => {
        try {
            const client = new client_resource_groups_tagging_api_1.ResourceGroupsTaggingAPIClient({ region: region, credentials: credential });
            const input = {};
            const command = new client_resource_groups_tagging_api_1.GetTagKeysCommand(input);
    ...
    'KexaAwsCustoms.resourcesTags': async (credential, region, object) => {
        try {
            const client = new client_resource_groups_tagging_api_1.ResourceGroupsTaggingAPIClient({ region: region, credentials: credential });
            const input = {};
            const command = new client_resource_groups_tagging_api_1.GetResourcesCommand(input);
...
async function tagsValueListing(client, command, region) {
    try {
...
async function complianceSummaryListing(client, command, region) {
    try {
        const dataKeys = await client.send(command);
...
//# sourceMappingURL=data:application/json;base64,
